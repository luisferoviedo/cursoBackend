...
function App() {
  // authStatus controla el guard global del frontend:
  // checking -> todavía no sabemos si hay sesión válida
  // guest -> no hay sesión activa
  // authenticated -> la app ya puede mostrar rutas privadas
  ...
  useEffect(() => {
    ...
    setUnauthorizedHandler(() => {
      clearClientSession('Tu sesion vencio. Vuelve a entrar para continuar')
    })
  ...
  async function loadCurrentUser(token) {
    try {
      ...
      // Esto evita confiar ciegamente en un token guardado que ya pudo expirar o ser inválido.
  ...
  async function handleLoginSuccess(authResult) {
    ...
    // Este punto es la transición real de pantalla pública a app autenticada.
  ...
  return (
    // App no decide vistas puntuales.
    // Solo prepara el estado y le entrega al router lo necesario para pintar cada ruta.
    <AppRouter
